...
namespace te
{
	...
	T UTF8To32(T begin, T end, char32_t& output, char32_t invalidChar = 0)
	{
		...
		switch(numBytes)
		{
		case 6: output += (UINT8)(*begin); ++begin; output <<= 6; TE_FALLTHROUGH;
		case 5: output += (UINT8)(*begin); ++begin; output <<= 6; TE_FALLTHROUGH;
		case 4: output += (UINT8)(*begin); ++begin; output <<= 6; TE_FALLTHROUGH;
		case 3: output += (UINT8)(*begin); ++begin; output <<= 6; TE_FALLTHROUGH;
		case 2: output += (UINT8)(*begin); ++begin; output <<= 6; TE_FALLTHROUGH;
		case 1: output += (UINT8)(*begin); ++begin; TE_FALLTHROUGH;
		default: break;
		}
	...
	T UTF32To8(char32_t input, T output, UINT32 maxElems, char invalidChar = 0)
	{
		...
		switch (numBytes)
		{
			case 4: bytes[3] = (char)((input | 0x80) & 0xBF); input >>= 6; TE_FALLTHROUGH;
			case 3: bytes[2] = (char)((input | 0x80) & 0xBF); input >>= 6; TE_FALLTHROUGH;
			case 2: bytes[1] = (char)((input | 0x80) & 0xBF); input >>= 6; TE_FALLTHROUGH;
			case 1: bytes[0] = (char)(input | headers[numBytes]); TE_FALLTHROUGH;
			default: break;
		}
