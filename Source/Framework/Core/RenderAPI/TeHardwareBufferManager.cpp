...
namespace te
{
    TE_MODULE_STATIC_MEMBER(HardwareBufferManager)
    ...
    SPtr<GpuBuffer> HardwareBufferManager::CreateGpuBuffer(const GPU_BUFFER_DESC& desc,
        GpuDeviceFlags deviceMask)
    {
        SPtr<GpuBuffer> gpuBuffer = CreateGpuBufferInternal(desc, deviceMask);
    ...
    {
        SPtr<GpuBuffer> gpuBuffer = CreateGpuBufferInternal(desc, std::move(underlyingBuffer));
        gpuBuffer->Initialize();
